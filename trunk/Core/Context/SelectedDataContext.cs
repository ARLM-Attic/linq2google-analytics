...
namespace GoogleAnalyticsLinqProvider.Core
{
    ...
    public class SelectedDataContext<T>
    {
        ...
        public List<T> ToList(int? skip = null, int? take = null)
        {
            ...
            return Factory.GetAnalyticsProcessor<T>().Execute(_queryData, _expression, _fieldsMap).ToList();
        }
        ...
        public T First(long? skip = null)
        {
            _queryData.Skip = skip;
            _queryData.Take = 1;
            return Factory.GetAnalyticsProcessor<T>().Execute(_queryData, _expression, _fieldsMap).FirstOrDefault();
        }
        ...
        public void ToListAsync(Action<List<T>> callback, int? skip = null, int? take = null)
        {
            _queryData.Skip = skip;
            _queryData.Take = take;
            Factory.GetAnalyticsProcessor<T>().ExecuteAsync(_queryData, _expression, _fieldsMap, x => callback(x.ToList()));
        }
        ...
        public void FirstAsync(Action<T> callback, long? skip = null)
        {
            _queryData.Skip = skip;
            _queryData.Take = 1;
            Factory.GetAnalyticsProcessor<T>().ExecuteAsync(_queryData, _expression, _fieldsMap, x => callback(x.First()));
        }
        #endregion
    }
}
